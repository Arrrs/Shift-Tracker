-- Fix auto-income trigger to handle job deletion properly
-- When a job is deleted, it sets job_id=NULL on time_entries via CASCADE
-- The trigger must handle this case without throwing "v_job is not assigned" error
--
-- Solution: Use separate variables for job_id and job_name instead of accessing
-- the RECORD type directly, which can be uninitialized
...
DECLARE
  ...
  v_job_currency TEXT;
  v_job_pay_type TEXT;
  v_job_hourly_rate DECIMAL(10,2);
  v_job_daily_rate DECIMAL(10,2);
...
BEGIN
  ...
  IF NEW.job_id IS NOT NULL THEN
    SELECT
      id, name, currency, pay_type, hourly_rate, daily_rate
    INTO
      v_job_id, v_job_name, v_job_currency, v_job_pay_type, v_job_hourly_rate, v_job_daily_rate
    FROM jobs
    WHERE id = NEW.job_id;

    -- If job was found, use its currency
    IF FOUND THEN
      v_currency := v_job_currency;
    ELSE
      -- Job doesn't exist (might have been just deleted)
      v_currency := 'USD';
      v_job_id := NULL;
    END IF;
  ELSE
  ...
  -- Treat "default", "none", empty string as NULL (no override)
  IF NEW.pay_override_type IS NOT NULL
     AND NEW.pay_override_type != 'default'
     AND NEW.pay_override_type != 'none'
     AND NEW.pay_override_type != '' THEN
  ...
  ELSIF v_job_id IS NOT NULL AND v_job_pay_type IN ('hourly', 'daily') THEN
    v_pay_type := v_job_pay_type;
  ...
  CASE v_pay_type
    ...
    WHEN 'holiday_multiplier' THEN
      ...
      ELSIF v_job_id IS NOT NULL AND v_job_pay_type = 'hourly' AND v_job_hourly_rate IS NOT NULL THEN
        v_base_rate := v_job_hourly_rate;
        v_calculated_amount := NEW.actual_hours * v_base_rate * NEW.holiday_multiplier;
        v_calculation_basis := jsonb_build_object(
          'type', 'holiday_multiplier_hourly',
          'hours', NEW.actual_hours,
      ...
      ELSIF v_job_id IS NOT NULL AND v_job_pay_type = 'daily' AND v_job_daily_rate IS NOT NULL THEN
        v_base_rate := v_job_daily_rate;
        v_calculated_amount := v_base_rate * NEW.holiday_multiplier;
        v_calculation_basis := jsonb_build_object(
          'type', 'holiday_multiplier_daily',
          'base_rate', v_base_rate,
    ...
    -- Job default hourly
    WHEN 'hourly' THEN
      IF v_job_id IS NULL OR v_job_hourly_rate IS NULL THEN
        RETURN NEW;
      END IF;

      v_calculated_amount := NEW.actual_hours * v_job_hourly_rate;
      v_calculation_basis := jsonb_build_object(
        'type', 'job_hourly',
        'hours', NEW.actual_hours,
        'rate', v_job_hourly_rate,
        'job_id', v_job_id,
        'job_name', v_job_name,
        'is_holiday', NEW.is_holiday,
        'formula', NEW.actual_hours || 'h × $' || v_job_hourly_rate
      );

    -- Job default daily
    WHEN 'daily' THEN
      IF v_job_id IS NULL OR v_job_daily_rate IS NULL THEN
        RETURN NEW;
      END IF;

      v_calculated_amount := v_job_daily_rate;
      v_calculation_basis := jsonb_build_object(
        'type', 'job_daily',
        'rate', v_job_daily_rate,
        'job_id', v_job_id,
        'job_name', v_job_name,
        'is_holiday', NEW.is_holiday,
        'formula', 'Daily rate: $' || v_job_daily_rate
      );
...
Properly handles NULL job_id cases for freelance work and job deletion scenarios.
Treats pay_override_type values of "default", "none", and empty string as no override.
Uses individual variables instead of RECORD type to avoid uninitialized record errors.
