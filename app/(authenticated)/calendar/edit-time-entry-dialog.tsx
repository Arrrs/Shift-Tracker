...
export function EditTimeEntryDialog({ open, onOpenChange, entry, onSuccess }: EditTimeEntryDialogProps) {
  ...
  // Form state
  const [entryType, setEntryType] = useState<"work_shift" | "day_off">(entry?.entry_type || "work_shift");
  ...
  const [customHourlyRate, setCustomHourlyRate] = useState<string>(entry?.custom_hourly_rate?.toString() || "");
